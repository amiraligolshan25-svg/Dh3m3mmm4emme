...
import os
import random
from telegram import Update, InlineKeyboardButton, InlineKeyboardMarkup, ForumTopicEdited
...
DEFAULT_COUNTRIES = {
    ...
    "usa": {
        ...
        "equipment": {"tanks": 15000, "fighters": 12000, "ships": 280, "missiles": 320, "nukes": 3, "guns": 0, "ammo": 0}
    },
    "soviet": {
        ...
        "equipment": {"tanks": 18000, "fighters": 7500, "ships": 45, "missiles": 90, "nukes": 0, "guns": 0, "ammo": 0}
    },
    "uk": {
        "name": "بریتانیا",
        ...
        "equipment": {"tanks": 4200, "fighters": 5100, "ships": 210, "missiles": 140, "nukes": 0, "guns": 0, "ammo": 0}
    },
    "japan": {
        "name": "امپراتوری ژاپن",
        "leader": "هیروهیتو",
        ...
        "equipment": {"tanks": 2800, "fighters": 3800, "ships": 160, "missiles": 70, "nukes": 0, "guns": 0, "ammo": 0}
    },
    "italy": {
        "name": "ایتالیا",
        ...
        "equipment": {"tanks": 1600, "fighters": 1400, "ships": 55, "missiles": 25, "nukes": 0, "guns": 0, "ammo": 0}
    },
    "iran": {
        "name": "ایران",
        ...
        "equipment": {"tanks": 180, "fighters": 95, "ships": 6, "missiles": 5, "nukes": 0, "guns": 0, "ammo": 0}
    },
    "indonesia": {
        "name": "اندونزی",
        "leader": "سوکارنو",
        ...
        "equipment": {"tanks": 650, "fighters": 320, "ships": 28, "missiles": 15, "nukes": 0, "guns": 0, "ammo": 0}
    },
    "north_korea": {
        "name": "کره شمالی",
        "leader": "کیم ایل‌سونگ",
        ...
        "equipment": {"tanks": 2200, "fighters": 480, "ships": 18, "missiles": 85, "nukes": 0, "guns": 0, "ammo": 0}
    },
    "afghanistan": {
        "name": "افغانستان",
        ...
        "equipment": {"tanks": 85, "fighters": 35, "ships": 0, "missiles": 2, "nukes": 0, "guns": 0, "ammo": 0}
    },
    "china": {
        "name": "چین",
        ...
        "equipment": {"tanks": 4800, "fighters": 2100, "ships": 45, "missiles": 40, "nukes": 0, "guns": 0, "ammo": 0}
    },
    "india": {
        "name": "هند",
        ...
        "equipment": {"tanks": 2100, "fighters": 850, "ships": 32, "missiles": 25, "nukes": 0, "guns": 0, "ammo": 0}
    },
    "israel": {
        "name": "اسرائیل",
        ...
        "equipment": {"tanks": 950, "fighters": 420, "ships": 12, "missiles": 55, "nukes": 100, "guns": 0, "ammo": 0}
    },
    "saudi": {
        "name": "عربستان سعودی",
        ...
        "equipment": {"tanks": 280, "fighters": 95, "ships": 8, "missiles": 12, "nukes": 0, "guns": 0, "ammo": 0}
    },
    "australia": {
        "name": "استرالیا",
        "leader": "جان کرتین",
        ...
        "equipment": {"tanks": 1100, "fighters": 950, "ships": 48, "missiles": 20, "nukes": 0, "guns": 0, "ammo": 0}
    }
}
...
async def army(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
      [InlineKeyboardButton(text=f"نیروی انسانی: {c['army']}",callback_data="nothing")],
      [
        InlineKeyboardButton(text=f"تانک: {e['tanks']}", callback_data="nothing"),
        InlineKeyboardButton(text=f"جنگنده: {e['fighters']}", callback_data="nothing")
      ],
      [
        InlineKeyboardButton(text=f"ناو: {e['ships']}", callback_data="nothing"),
        InlineKeyboardButton(text=f"موشک: {e['missiles']}", callback_data="nothing")
      ],
      [InlineKeyboardButton(text=f"هسته ای: {e['nukes']}", callback_data="nothing")]
    ]
...
async def resources(update: Update, context = ContextTypes.DEFAULT_TYPE):
  ...
  text = f"تولید ماهانه کشور {name}📅"
  keyboard = [
    [
      InlineKeyboardButton(text = f"نفت: {COUNTRIES[country]['resources']['oil']}", callback_data= "nothing"),
      InlineKeyboardButton(text = f"فولاد: {COUNTRIES[country]['resources']['steel']}", callback_data= "nothing")
    ],
    [
      InlineKeyboardButton(text = f"زغال: {COUNTRIES[country]['resources']['coal']}", callback_data= "nothing"),
      InlineKeyboardButton(text = f"غذا: {COUNTRIES[country]['resources']['food']}", callback_data= "nothing")
    ],
    [
      InlineKeyboardButton(text = f"اورانیوم: {COUNTRIES[country]['resources']['uranium']}", callback_data= "nothing"),
      InlineKeyboardButton(text = f"لاستیک: {COUNTRIES[country]['resources']['rubber']}", callback_data= "nothing")
    ]
  ]
...
async def collect_resources(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  if not country in COUNTRIES:
    await update.message.reply_text("اول کشورت را انتخاب کن")
    return
  
  c = COUNTRIES[country]
  now = time.time()
  last_resources_collect = c['last_resources_collect']
  cooldown = 60 * 720

  remaining = int(cooldown - (now - last_resources_collect))
  if remaining > 0:
      minutes = remaining // 60
      seconds = remaining % 60
      await update.message.reply_text(
      ...
      return
  p = COUNTRIES[country]['production']
  resources = COUNTRIES[country]['resources']
  oil = resources['oil'] 
  ...
  stockpile = COUNTRIES[country]['stockpile']
  e = COUNTRIES[country]['equipment']
  
  stockpile['oil'] += oil
  stockpile['steel'] += steel
  stockpile['coal'] += coal
  stockpile['food'] += food
  stockpile['rubber'] += rubber
  stockpile['uranium'] += uranium
  e['tanks'] += tanks
  e['ships'] += ships
  e['guns'] += guns
  e['ammo'] += ammo
...
async def stockpile(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  keyboard = [
    [
      InlineKeyboardButton(text = f"نفت: {COUNTRIES[country]['stockpile']['oil']}", callback_data= "nothing"),
      InlineKeyboardButton(text = f"فولاد: {COUNTRIES[country]['stockpile']['steel']}", callback_data= "nothing")
    ],
    [
      InlineKeyboardButton(text = f"زغال: {COUNTRIES[country]['stockpile']['coal']}", callback_data= "nothing"),
      InlineKeyboardButton(text = f"غذا: {COUNTRIES[country]['stockpile']['food']}", callback_data= "nothing")
    ],
    [
      InlineKeyboardButton(text = f"اورانیوم: {COUNTRIES[country]['stockpile']['uranium']}", callback_data= "nothing"),
      InlineKeyboardButton(text = f"لاستیک: {COUNTRIES[country]['stockpile']['rubber']}", callback_data= "nothing")
    ]
  ]
  mark_up = InlineKeyboardMarkup(keyboard)
  await update.message.reply_text(text, reply_markup= mark_up)
...
async def pay(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  country = get_user_country(uid)
  if not country in COUNTRIES:
    await update.message.reply_text("اول کشور خود را انتخاب کن")
    return
  if not context.args:
    await update.message.reply_text("مثال: /pay target amount")
    return
  target = context.args[0]
  amount = int(context.args[1])
  targ = COUNTRIES[target]
  if not target in COUNTRIES:
    await update.message.reply_text("کشور نامعتبر")
    return
  COUNTRIES[country]['war_credit'] -= amount

  COUNTRIES[country]['war_credit'] += amount
  await update.message.reply_text(f"مقدار {amount} WAR_CREDIT از {COUNTRIES[country]['name']} به {COUNTRIES[target]['name']} ارسال شد")
...
def main():
    ...
    app.add_handler(CommandHandler("stockpile", stockpile))
    app.add_handler(CommandHandler("resources", resources)) 
    app.add_handler(CommandHandler("resources_collect", collect_resources))  
